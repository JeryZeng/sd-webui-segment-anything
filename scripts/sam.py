...
def get_sam_embedding(sam_model_name, input_image: Image) -> (np.ndarray, str):
    print("Start SAM Processing")
    if sam_model_name is None:
        return None, "SAM model not found. Please download SAM model from extension README."
    if input_image is None:
        return None, "SAM requires an input image. Please upload an image first."
    image_np = np.array(input_image)
    image_np_rgb = image_np[..., :3]
    sam = init_sam_model(sam_model_name)
...
def get_sam_masks(sam_model_name, input_image: Image, compress: bool, min_area=100) -> (np.ndarray, str):
    ...
    def convert_ann2struct(ann, box: [], area):
        ...
        # 去掉尾部多余的
        nonlocal h, w
        box_end_x = int(box[0] + box[2])
        box_end_y = int(box[1] + box[3])
        if box_end_x < w - 1:
            ann = np.delete(ann, np.arange(box_end_x, w), 1)
        if box_end_y < h - 1:
            ann = np.delete(ann, np.arange(box_end_y, h), 0)
        # 去掉头部多余的
        box_x = int(box[0])
        box_y = int(box[1])
        if box_x > 0:
            ann = np.delete(ann, np.arange(0, box_x + 1), 1)
        if box_y > 0:
            ann = np.delete(ann, np.arange(0, box_y + 1), 0)
        ...
        if compress:
            ...
            ann_h, ann_w = ann.shape
            for row_index in range(ann_h):
                valid_point = False
                row = []
                ...
                for col_index in range(col_start_index, ann_w):
